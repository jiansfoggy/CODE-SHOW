...
class Conv2dStaticSamePadding(nn.Conv2d):
    ...
    def forward(self, x):
        ...
        #print("x, self.weight, self.bias: ", np.shape(x), np.shape(self.weight),np.shape(self.bias))
        #print(np.shape(self.stride), np.shape(self.padding), np.shape(self.dilation), np.shape(self.groups))
        x = F.conv2d(x, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        #print("once come here: ")
        return x
